...
public class Matrix {
        ...
        public function inverse() {
                ...
                for(i = copy.rows - 1; i > 0; i--) {                   
                	lower_i = identity.getRow(i);
                    lower = copy.getRow(i);
                   	denominator = lower[i];

                    for(j = i; j > 0; j--) {
	                    upper = copy.getRow(j-1);
	                    upper_i = identity.getRow(j-1);
                     	numerator = upper[i];
	                    
	                    for(k = 0; k < copy.cols; k++) {
	                    	element = lower[k];
	                    	element2 = upper[k];
	                        upper[k] = element2 - numerator/denominator*element;
	                     	element = lower_i[k];
	                    	element2 = upper_i[k];
	                        upper_i[k] = element2 - numerator/denominator*element;
	                    }
	                    
	                    copy.setRow(j-1, upper);
	                    identity.setRow(j-1, upper_i);
                    }                    
                }
